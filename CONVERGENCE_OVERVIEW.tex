...
\node[anchor=north west, text=ink!75, font=\large, text width=49.8cm] at (0,-1.05)
  {A qubit's Hamiltonian acts on an infinite-dimensional Hilbert space; \texttt{scqubits} represents it as a \emph{finite matrix} in a truncated basis.
   \texttt{check\_convergence} asks: \emph{for this cutoff, is there any reason to distrust the result --- and what should I grow if there is?}};
...
\node[zone=flow, anchor=north west] (B) at (\colB,\rowA) {\body{%
  ...
  \vspace{2.55cm}\par
  \bk{\cd{report = q.check\_convergence(n\_levels, target\_abs\_GHz, mode)}}\par
...
\node[pill=flow, right=0.55cm of w1] (w2) {\cd{check\_convergence}};
