...
class ExplorerSetup:
    ...
    def __init__(self, sweep: scq.ParameterSweep):
        ...
        self.ui_panels_list = Select(
            ...
            rows=10,
            value=self.selected_as_strings()[-1],
        )
        ...
        self.ui_vbox["settings"] = VBox(
            [
                HTML("<b>Settings: {}</b>".format(self.ui_panels_list.value)),
    ...
    def display_panel(
        ...
        if panel_name == "Transitions":
            ...
            subsys_name = self.ui_transitions["highlight_dropdown"].value
            subsys = self.sweep.subsys_by_id_str(subsys_name)
    ...
    def activate_settings(self, change):
        subsys_name, panel_name = self.ui_panels_list.value.split(SEP)
    ...
    def create_ui_settings_subsys(self, subsys_index: int, panel_name: str):
        if panel_name == "Energy spectrum":
            ...
            ui_subtract_ground_checkbox = Checkbox(
                description="subtract lowest energy", value=False, layout=l300px
            )
    ...
    def create_ui_settings_composite(self, panel_name: str):
        if panel_name == "Transitions":
            ...
            self.ui_transitions["photons_inttext"] = BoundedIntText(
                value=1, min=1, max=5, description="photons", layout=l185px
            )
            self.ui_transitions["highlight_dropdown"] = Dropdown(
                description="Highlight transitions for:",
                options=self.subsys_names,
                layout=l185px,
            )
            self.ui_transitions["sidebands_checkbox"] = Checkbox(
                description="show sidebands", value=False, layout=l185px
            )
            for initial_inttext in self.ui_transitions["initial_state_inttexts"]:
                initial_inttext.observe(self.update_explorer, "value")
            self.ui_transitions["photons_inttext"].observe(
                self.update_explorer, "value"
            )
            self.ui_transitions["highlight_dropdown"].observe(
                self.update_explorer, "value"
            )
            ...
            return [
                HBox(
                    [
                        HTML(value="Initial state (bare product state)"),
                        *self.ui_transitions["initial_state_inttexts"],
                    ]
                ),
                self.ui_transitions["photons_inttext"],
                self.ui_transitions["highlight_dropdown"],
