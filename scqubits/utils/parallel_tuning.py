...
# across worker processes. Tied to the auto-sparse threshold.
_LARGE_DENSE_DIM = settings.SPARSE_DIAG_MIN_DIM
...
def _default_is_sparse(dimension: int, evals_count: int) -> bool:
    ...
    return bool(
        ...
        and dimension >= settings.SPARSE_DIAG_MIN_DIM
        and evals_count <= max(1, int(dimension * settings.SPARSE_DIAG_MAX_EVALS_FRAC))
    )
