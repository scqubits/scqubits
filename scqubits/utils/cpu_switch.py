            ...
            def submit_until_full():
                ...
                while not iterable_done and len(pending) < max_pending:
                    try:
                        ...
                        break
                    pending.append(
                        remote_func.remote(func_ref_box, next_submit_index, x)
                    )
                    next_submit_index += 1
...
def get_map_method(
    ...
    cpu_per_node: Optional[int] = None,
) -> Callable:
    """Selects the correct `.map` method depending on the specified number of desired
    cores. If num_cpus>1, a multiprocessing/pathos/ray backend is used.
    ...
    ``settings.MULTIPROC``), or a Ray-backed generator map. ``imap`` (not ``map``)
    is used for pool backends so the caller can wrap the returned iterator in
    ``tqdm`` and get a live progress bar that advances as workers finish, while
    results are still yielded in input order.
    """
    if num_cpus == 1:
        return map

    if settings.MULTIPROC == "ray":
        return _make_ray_map(num_cpus, cpu_per_node=cpu_per_node)
