...
class Grid1d(dispatch.DispatchClient, serializers.Serializable):
    ...
    def first_derivative_matrix(self, prefactor=1.0, periodic=False):
        ...
        matrix_diagonals = [coefficient * prefactor / 2 * delta_x for coefficient in FIRST_STENCIL_COEFFS[STENCIL]]
        offset = [i - (STENCIL - 1) / 2 for i in range(STENCIL)]
        derivative_matrix = self.band_matrix(matrix_diagonals, offset, self.pt_count, dtype=dtp, has_corners=periodic)
    ...
    def band_matrix(self, band_coeffs, band_offsets, dim, dtype=None, has_corners=False):
        """
        Returns a dim x dim sparse matrix with `band_vecs[0]`, `band_vecs[1]`, ... along the (off-)diagonals specified
        by the offsets `band_offsets[0]`, `band_offsets[1]`, ... The `has_corners` option allows generation of band
        matrices with corner elements, in which lower off-diagonals wrap into the top right corner and upper
        ...
        ones_vector = np.ones(self.pt_count)
