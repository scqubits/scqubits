...
class ZeroPiNoGrid(base.QubitBaseClass, serializers.Serializable):
    def __init__(
        ...
        self._image_filename = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "qubit_pngs/zeropi.png"
        )
    ...
    def E_phi_osc(self):
        return np.sqrt(
            8.0 * self.ECJ * self.EL
        )  # since H_harm_phi = 2E_{CJ}n_{\phi}^2 + E_{L}\phi^2

    def phi_osc(self):
        return (
            2.0 * self.ECJ / self.EL
        ) ** 0.25  # since H_harm_phi = 2E_{CJ}n_{\phi}^2 + E_{L}\phi^2
    ...
    def cos_theta_operator(self):
        return 0.5 * (
            self.exp_i_theta_operator() + self.exp_i_theta_operator().conj().T
        )
    ...
    def hamiltonian(self):
        ...
        cos_phi = 0.5 * (
            np.exp(-1j * 2.0 * np.pi * self.flux / 2.0) * exp_i_phi
            + np.exp(1j * 2.0 * np.pi * self.flux / 2.0) * exp_i_phi.conj().T
        )
        potential = (
            2.0
            * self.EJ
            * (
                np.kron(identity_phi, identity_theta)
                - np.kron(cos_phi, self.cos_theta_operator())
            )
        )

        hamiltonian = (
            np.kron(phi_harmonic, identity_theta)
            + np.kron(identity_phi, theta_kinetic)
            + potential
            + 0.5 * self.E_phi_osc() * np.kron(identity_phi, identity_theta)
        )
        return hamiltonian
