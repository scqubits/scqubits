...
class FluxoniumTunableCouplerFloating(serializers.Serializable):
    ...
    def schrieffer_wolff_born_oppenheimer_effective_hamiltonian(self):
        ...
        H_1 = sum((V_op(init_a, fin_a, init_b, fin_b))
...
#        flux_a, flux_b = 2.0 * np.pi * self.flux_a - flux_shift, 2.0 * np.pi * self.flux_b - flux_shift
    ...
    def _single_hamiltonian_effective(fluxonium_instance, hilbert_space):
        ...
        phi_ops = sum([phi_mat[j][k] * hilbert_space.hubbard_operator(j, k, fluxonium_instance)
                       for j in range(dim) for k in range(dim)])
        n_ops = sum([n_mat[j][k] * hilbert_space.hubbard_operator(j, k, fluxonium_instance)
                     for j in range(dim) for k in range(dim)])
...
class FluxoniumTunableCouplerGrounded(FluxoniumTunableCouplerFloating, serializers.Serializable, base.QubitBaseClass):
    def __init__(self, EJa, EJb, EC_twoqubit, ECq1, ECq2, ELa, ELb, flux_a, flux_b, flux_c,
                 fluxonium_cutoff, fluxonium_truncated_dim, ECc, ECm, EL1, EL2, EJC,
                 fluxonium_minus_truncated_dim=6, h_o_truncated_dim=3):
        FluxoniumTunableCouplerFloating.__init__(self, EJa, EJb, np.inf, 0.0, 0.0, ECq1, ECq2, ELa, ELb,
                                                 flux_a, flux_b, flux_c, fluxonium_cutoff, fluxonium_truncated_dim,
                                                 ECc, ECm, EL1, EL2, EJC,
