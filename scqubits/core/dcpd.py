...
class Dcpd(base.QubitBaseClass, serializers.Serializable):
    ...
    def theta_osc(self):
        ...
        return (8 * self.EC * self.x / (2 * self.EL + self.ELA)) ** 0.25

    def varphi_osc(self):
        """Return the oscillator strength of varphi degree of freedom"""
        return (4 * self.EC / self.ELA) ** 0.25
    ...
    def theta_plasma(self):
        ...
        """
        return math.sqrt(8.0 * self.x * self.EC * (2 * self.EL + self.ELA))
    ...
    def varphi_plasma(self):
        ...
        """
        return math.sqrt(4.0 * self.ELA * self.EC)
    ...
    def _varphi_operator(self):
        return sparse.dia_matrix((self.varphi_grid.make_linspace(), [0]), shape=(self.dim_varphi(), self.dim_varphi())).tocsc()
    ...
    def hamiltonian(self):
        ...
        cross_mat = - 4 * self.EC * self.n_theta_operator() * self.n_varphi_operator()

        theta_osc_mat = self._kron3(self._identity_phi(), op.number_sparse(self.dim_theta(), self.theta_plasma()),
                                    self._identity_varphi()) + 2 * self.EC * self.n_theta_operator() ** 2

        phi_inductive_mat = 0.25 * self.EL * self.phi_operator() ** 2
    ...
    def get_t1_capacitive_loss(self, init_state):
        ...
        s_vv_1 = 2 * np.pi * 16 * self.EC / self.q_cap(np.abs(energy_diff)) * self.thermal_factor(energy_diff)
        s_vv_2 = 2 * np.pi * 16 * self.EC / self.q_cap(np.abs(energy_diff)) * self.thermal_factor(energy_diff)
    ...
    def get_t1_inductive_loss(self, init_state):
        ...
        s_ii_1 = 2 * np.pi * 2 * self.EL / self.q_ind(np.abs(energy_diff)) * self.thermal_factor(energy_diff)
        s_ii_2 = 2 * np.pi * 2 * self.EL / self.q_ind(np.abs(energy_diff)) * self.thermal_factor(energy_diff)
