...
class ZeroPiVTB(
    vtb.VTBBaseMethods, ZeroPi, base.QubitBaseClass, serializers.Serializable
):
    r"""Zero Pi using VTB
    ...
    def _check_second_derivative_positive(self, phi: ndarray, theta: ndarray) -> bool:
        return (
            self.EL + 2 * self.EJ * np.cos(theta) * np.cos(phi - np.pi * self.flux)
        ) > 0
    ...
    def gamma_matrix(self, minimum_index: int = 0) -> ndarray:
        ...
        Phi0 = 1.0 / (2 * 1.0)  # units where e_charge = 1.
        ...
        gamma_matrix[0, 0] = (
            ...
            * np.cos(theta_location)
            - 2
            * self.dEJ
            * np.sin(theta_location)
            * np.sin(phi_location - np.pi * self.flux)
        )
        gamma_matrix[1, 1] = 2 * self.EJ * np.cos(
            phi_location - np.pi * self.flux
        ) * np.cos(theta_location) - 2 * self.dEJ * np.sin(theta_location) * np.sin(
            phi_location - np.pi * self.flux
        )
        off_diagonal_term = (
            -2
    ...
    def _BCH_factor(self, j: int, Xi: ndarray) -> ndarray:
        ...
        return np.exp(
            ...
            * np.sum(
                [
                    cosine_coeffs[i] * cosine_coeffs[k] * np.dot(Xi[i, :], Xi.T[:, k])
                    for i in range(dim)
                    for k in range(dim)
                ]
            )
    ...
    def _single_exp_i_phi_j_operator(
        ...
        exp_i_phi_theta_a_component = expm(
            np.sum(
                [
                    1j * cosine_coeffs[i] * Xi[i, k] * a_operator_list[k] / np.sqrt(2.0)
                    for i in range(dim)
                    for k in range(dim)
                ],
                axis=0,
    ...
    def _harmonic_contribution_to_potential(
        self, premultiplied_a_and_a_dagger: Tuple, Xi: ndarray, phi_bar: ndarray
    ) -> ndarray:
        dim = self.number_degrees_freedom
        a, a_a, a_dagger_a = premultiplied_a_and_a_dagger
        harmonic_contribution = np.sum(
            [
                0.5
                * self.EL
                * Xi[0, i]
                * Xi.T[i, 0]
                * (a_a[i] + a_a[i].T + 2.0 * a_dagger_a[i] + self._identity())
    ...
    def _exp_i_phi_theta_with_phi_bar(
        self, j: int, exp_i_phi_j: ndarray, phi_bar: ndarray
    ) -> ndarray:
        dim = self.number_degrees_freedom
        cosine_coeffs = np.array([(-1) ** j, 1])
        exp_i_phi_theta_with_phi_bar = (
            exp_i_phi_j[j]
            * np.prod([np.exp(1j * cosine_coeffs[i] * phi_bar[i]) for i in range(dim)])
...
class ZeroPiVTBSqueezing(vtbs.VTBBaseMethodsSqueezing, ZeroPiVTB):
    ...
    def _local_potential_squeezing(
        ...
        for j in range(dim):
            cosine_coeffs = np.array([(-1) ** j, 1])
            exp_i_phi_list.append(
                exp_i_list[j]
                * np.prod(
                    [np.exp(1j * cosine_coeffs[i] * phi_bar[i]) for i in range(dim)]
                )
                * np.exp((-1) ** (j + 1) * 1j * np.pi * self.flux)
            )
        ...
        potential_matrix += self._local_potential_harmonic_squeezing(
            Xi,
            Xi_inv,
            displacement_vector,
            minima_m,
    ...
    def _minima_pair_potential_squeezing(
        ...
        linear_coefficients_potential = self._linear_coefficient_matrices(
            rho_prime, delta_rho, Xi / np.sqrt(2.0), Xi / np.sqrt(2.0)
        )
        return (
            self._potential_operators_squeezing(
                a_operator_list,
                Xi,
                exp_a_dagger_a,
                disentangled_squeezing_matrices,
            ...
            self._minima_pair_potential_harmonic_squeezing(
                a_operator_list,
                exp_a_dagger_a,
                disentangled_squeezing_matrices,
                delta_rho_matrices,
                linear_coefficients_potential,
            ),
        )

    def _minima_pair_potential_harmonic_squeezing(
        self,
        a_operator_list: ndarray,
        exp_a_dagger_a: ndarray,
        disentangled_squeezing_matrices: Tuple,
        ...
        (xa, xaa, dxa, dx, ddx) = self._premultiplying_exp_a_dagger_a_with_a(
            exp_a_dagger_a, a_operator_list
        )
        sigma_delta_rho_bar_zpp = (
            expm(-sigma).T @ expm(delta_rho_bar) @ a_dagger_coefficient.T
        )[:, 0]
        xaa_coefficient = np.outer(
            (a_coefficient @ expm(-sigma_prime)).T[:, 0],
            (a_coefficient @ expm(-sigma_prime))[0, :],
        )
        dxa_coefficient = np.outer(
            sigma_delta_rho_bar_zpp, (a_coefficient @ expm(-sigma_prime))[0, :]
        )
        ddx_coefficient = np.outer(
            sigma_delta_rho_bar_zpp,
            (expm(-sigma).T @ expm(delta_rho_bar) @ a_dagger_coefficient.T).T[0, :],
        )
        x_coefficient = a_dagger_coefficient.T[:, 0] @ a_coefficient[0, :]
        xa_coefficient = a_coefficient[0, :] @ expm(-sigma_prime)
        dx_coefficient = (
            a_dagger_coefficient[0, :] @ (expm(-sigma).T @ expm(delta_rho_bar)).T
        )
        potential_matrix = (
            np.sum(
                [
                    xaa_coefficient[mu, nu]
                    * exp_a_dagger_a
                    @ a_operator_list[mu]
                    @ a_operator_list[nu]
                    + (
                        2
                        * dxa_coefficient[mu, nu]
                        * a_operator_list[mu].T
                        @ exp_a_dagger_a
                        @ a_operator_list[nu]
                    )
                    + (
                        ddx_coefficient[mu, nu]
                        * a_operator_list[mu].T
                        @ a_operator_list[nu].T
                        @ exp_a_dagger_a
                    )
                    for mu in range(dim)
                    for nu in range(dim)
    ...
    def _local_single_junction_squeezing(
        ...
        delta_rho, delta_rho_prime, delta_rho_bar = delta_rho_matrices
        cosine_coeffs = np.array([(-1) ** j, 1])
        arg_exp_a_dag = (delta_phi @ Xi_inv.T + 1j * (cosine_coeffs @ Xi)) / np.sqrt(
            2.0
        )
        alpha = self._alpha_helper(
            arg_exp_a_dag, -arg_exp_a_dag.conjugate(), rho_prime, delta_rho
        )
        potential_matrix = (
            -0.5
            * self.EJlist[j]
    ...
    def _construct_potential_alpha_epsilon_squeezing(
        self,
        Xi: ndarray,
        Xi_inv: ndarray,
        delta_phi: ndarray,
        ...
        epsilon = (
            -(
                A @ rho_prime @ arg_exp_a
                + 0.5
                * (B - A @ rho_prime)
                @ (delta_rho + delta_rho.T)
                @ (arg_exp_a_dag - rho_prime @ arg_exp_a)
            )
            + phi_bar
        )
        return alpha, epsilon

    def _local_potential_harmonic_squeezing(
        self,
        Xi: ndarray,
        Xi_inv: ndarray,
        ...
        delta_rho, delta_rho_prime, delta_rho_bar = delta_rho_matrices
        (
            potential_matrix_minima_pair,
            xa,
            dx,
            xa_coefficient,
            dx_coefficient,
        ) = minima_pair_results
        alpha, epsilon = self._construct_potential_alpha_epsilon_squeezing(
            Xi, Xi_inv, delta_phi, rho_prime, delta_rho, phi_bar
        )
        ...
        return alpha * (
            np.sum(
                [
                    2 * xa[mu] * e_xa_coefficient[mu]
                    + 2 * dx[mu] * e_dx_coefficient[mu]
                    for mu in range(dim)
                ],
                axis=0,
            )
            * self.EL
            + potential_matrix_minima_pair
            + exp_a_dagger_a * self.EL * epsilon[0] ** 2
        )
