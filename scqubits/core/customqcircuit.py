...
class branch:
    ...
    def __str__(self):
        return "Branch " + self.type + " connecting nodes: (" + str(self.nodes[0].id) + "," + str(self.nodes[1].id) + "); " + str(self.parameters)
    def __repr__(self):
        return "Branch({}, {}, {})".format(self.type, self.nodes[0].id, self.nodes[1].id)
...
class CustomQCircuit:
    ...
    def __init__(self, list_nodes: list, list_branches: list = None, mode: str = "sym"):
        ...
        self.H = None
        self.L = None
        self.L_old = None # Lagrangian in terms of untransformed generalized flux variables
    ...
    def new_basis(self):
        ...
        ind_nodes = [list(set(sum([i.nodes for i in t], ()))) for t in trees] # finding the nodes in each of the independent sets of branches found earlier

        ref = list(range(1,len(nodes)+1)) # using numbers as variables, maximum needed is the number of nodes in the circuit

        for i in range(len(ind_nodes)):
            for n in ind_nodes[i]:
                n.marker = ref[i]

        pos = [] # identifies the positions of the sets of nodes connected by the branches in s_1; different numbers on two nodes indicates that they are not connected through any of the branches in s_1. 0 implies the node does not belong to any of the branches in s_1
        for i in nodes:
            pos.append(i.marker)
...
#         modes_num = pos.count(0) + max(pos) # total number of cyclic modes
        ...
        for i in range(max(pos)):
            if i == 0 and pos.count(0) == 0:
                ...
                periodic_modes.append([basis_params[0] if t == ref[i] else basis_params[1] for t in pos])
        if pos.count(0) > 0:
            for i in range(len(pos)):
                if pos[i]==0:
                    periodic_modes.append([basis_params[0] if t==i else basis_params[1] for t in range(len(pos))])
...
#         modes_num = pos.count(0) + max(pos) # total number of cyclic modes
        ...
        for i in range(max(pos)):
            if i == 0 and pos.count(0) == 0:
                ...
                zombie_modes.append([basis_params[0] if t == ref[i] else basis_params[1] for t in pos])
        if pos.count(0) > 0:
            for i in range(len(pos)):
                if pos[i]==0:
                    zombie_modes.append([basis_params[0] if t==i else basis_params[1] for t in range(len(pos))])
        ...
        for i in range(max(pos)):
            if i == 0 and pos.count(0) == 0:
                ...
                cyclic_modes.append([basis_params[0] if t == ref[i] else basis_params[1] for t in pos])
        ...
        for m in (periodic_modes + zombie_modes): # adding the ones which are periodic such that all vectors in modes are LI
...
#         standard_basis = np.ones([len(nodes),len(nodes)]) - 2*np.identity(len(nodes))
        ...
        pos_cyclic = [i for i in range(len(new_basis)) if i not in pos_Σ if new_basis[i].tolist() in cyclic_modes]
        pos_periodic = [i for i in range(len(new_basis)) if i not in pos_Σ if i not in pos_cyclic if new_basis[i].tolist() in periodic_modes]
        pos_zombie = [i for i in range(len(new_basis)) if i not in pos_Σ if i not in pos_cyclic if i not in pos_periodic if new_basis[i].tolist() in zombie_modes]
        pos_rest = [i for i in range(len(new_basis)) if i not in pos_Σ if i not in pos_cyclic if i not in pos_periodic if i not in pos_zombie]
        ...
        self.var_indices = {"cyclic":[i+1 for i in range(len(pos_list)) if pos_list[i] in pos_cyclic], 
                            "periodic":[i+1 for i in range(len(pos_list)) if pos_list[i] in pos_periodic], 
                            "discretized_phi":[i+1 for i in range(len(pos_list)) if pos_list[i] in pos_rest],
                            "zombie":[i+1 for i in range(len(pos_list)) if pos_list[i] in pos_zombie]}
        # updating param_vals
        if self.mode == "sym":
    ...
    def _C_matrix(self):
        ...
        for b in [t for t in self.branches if t.type=="C" or t.type=="JJ"]:
            if len(set(self.nodes)) > 1: # branch if shorted is not considered
                if b.type == "JJ":
                    C_mat[b.nodes[0].id - 1, b.nodes[1].id - 1] += -1/(b.parameters["E_CJ"]*8)
                else:
                    C_mat[b.nodes[0].id - 1, b.nodes[1].id - 1] += -1/(b.parameters["E_C"]*8)
    ...
    def _JJ_terms(self):
        ...
        for b in [t for t in self.branches if t.type == "JJ"]:
            if len(set(self.nodes)) > 1: # branch if shorted is not considered
                ...
                J += -b.parameters["E_J"] * sympy.cos(symbols("x" + str(b.nodes[1].id)) 
    ...
    def _L_terms(self):
        ...
        for b in [t for t in self.branches if t.type == "L"]:
            if len(set(self.nodes)) > 1: # branch if shorted is not considered
                ...
                L += 0.5 * b.parameters["E_L"] * (symbols("x" + str(b.nodes[1].id)) 
                                                     - symbols("x" + str(b.nodes[0].id)) + phi_ext)**2
        return L
    
    def _flux_loops(self):
        node_sets = [[self.nodes[0]]] # starting with the first set which has the first node as the only element

        i = 0
        while len([q for p in node_sets for q in p]) < len(self.nodes): # finding all the sets of nodes and filling node_sets
            node_set = []
            for n in node_sets[i]:
                node_set += (n.connected_nodes("L") + n.connected_nodes("JJ"))

            node_sets.append([x for x in list(set(node_set)) if x not in [q for p in node_sets[:i+1] for q in p]])
            i += 1
        ...
        for i in range(0, len(node_sets)):
            for n in node_sets[i]:
                ...
                if len(loop_branches) > 1:
                    flux_branches.append(loop_branches[: -1]) # selecting n-1 elements in the list for external flux
        
        # setting the class property
        if len(flux_branches) > 0:
            self.flux_branches = [i for j in flux_branches for i in j]
            self.external_flux_vars = [symbols("Φ" + str(i + 1)) for i in range(len(self.flux_branches))]
    ...
    def lagrangian_sym(self):
        ...
        y_vars = [symbols("y"+str(i)) for i in range(1,len(self.nodes)+1)] # defining the θ variables
...
#         L = (C_terms - L_terms - L_ex_flux_terms - JJ_terms).expand()
    ...
    def hamiltonian_sym(self):
        ...
        p_y_vars = [symbols('p'+str(i)) for i in range(1,len(self.nodes)+1)] # defining the momentum variables
        p_y = np.array([sympy.diff(L, i) for i in y_dot_vars]) # finding the momentum expression in terms of y_dot

        
        n_vars_p = len(self.var_indices["cyclic"] + self.var_indices["periodic"] + self.var_indices["discretized_phi"])
        y_dot_py = sympy.linsolve((p_y - np.array(p_y_vars)).tolist()[:n_vars_p], tuple(y_dot_vars[:n_vars_p]))
        y_dot_py = list(list(y_dot_py)[0])

        H = (p_y[:n_vars_p].dot(y_dot_vars[:n_vars_p]) - L).subs([(y_dot_vars[i], y_dot_py[i]) for i in range(len(y_dot_py))])

        for c in self.var_indices["cyclic"]: # To make it clear that the charge basis is used for cyclic variables
