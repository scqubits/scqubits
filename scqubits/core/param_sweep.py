...
class ParameterSweep(
    ...
    subsys_update_info:
        ...
        ``<subsystem a>`` and ``<subsystem b>``, leaving other subsystems
        unchanged; changes in ``<parameter name 2>`` affect none of the
        subsystems but require an update of the joint system; changes in
        ``<parameter name 3>`` affect none of the subsystems and the joint
        system (use ``None`` as the value).
    bare_only:
    ...
    def __init__(
        ...
        evals_count: int = 20,
        subsys_update_info: dict[str, list[QuantumSystem] | None] | None = None,
    ...
    def _dressed_spectrum_sweep(
        ...
        def _work_items() -> Any:
            for paramindex_tuple in itertools.product(*reduced_parameters.ranges):
                bare_esys = {
                    subsys_index: [
    ...
    def _dispersive_coefficients(
        ...
        NamedSlotsNdarray, NamedSlotsNdarray, NamedSlotsNdarray, NamedSlotsNdarray
    ]:
        ...
        Returns
        -------
        Tuple ``(lamb_data, chi_data, kerr_data, chi_prime_data)`` of
        :class:`NamedSlotsNdarray` instances. ``lamb_data`` is indexed by
        ``"subsys"``; ``chi_data``, ``kerr_data``, and ``chi_prime_data`` are
        indexed by ``("subsys1", "subsys2")``.
        """
...
def generator(sweep: "ParameterSweepBase", func: Callable, **kwargs) -> np.ndarray:
    ...
    **kwargs:
    ...
    target_map = cpu_switch.get_map_method(sweep._num_cpus, total=total_count)
